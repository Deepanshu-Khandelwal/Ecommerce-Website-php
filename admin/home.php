...
<body class="hold-transition skin-blue sidebar-mini">
        ...
        <div class="row">
          <div class="col-lg-3 col-xs-6">
            ...
            <div class="small-box bg-aqua">
              <div class="inner">
                <?php
                $stmt = $conn->prepare("SELECT SUM(amount) AS total FROM sales WHERE status = 'paid'");
                $stmt->execute();
                $srow = $stmt->fetch();
                $total = $srow['total'] ?? 0;
  ...
  <div class="small-box bg-red">
    <div class="inner">
      ...
      $stmt = $conn->prepare("
          ...
          FROM sales
          WHERE status = 'paid' AND DATE(sales_date) = CURDATE()
      ");
      $stmt->execute();
      $row = $stmt->fetch();
      $total = $row['total'] ?? 0;
  ...
  for ($m = 1; $m <= 12; $m++) {
    try {
      $stmt = $conn->prepare("SELECT SUM(amount) AS total FROM sales WHERE status = 'paid' AND MONTH(sales_date) = :month AND YEAR(sales_date) = :year");
