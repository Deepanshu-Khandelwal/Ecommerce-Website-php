...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 1. Fetch single row details (replacing products_row.php)
    if (isset($_POST['get_row'])) {
        $id = $_POST['id'] ?? 0;
        $stmt = $conn->prepare("SELECT *, products.id AS prodid, products.name AS prodname, category.name AS catname 
                                FROM products 
    ...
    // 2. Fetch all products options (replacing products_all.php)
    ...
    // 3. Add Product (replacing products_add.php)
    if (isset($_POST['add'])) {
        $name = $_POST['name'] ?? '';
        $slug = slugify($name);
    ...
    // 4. Edit Product (replacing products_edit.php)
    if (isset($_POST['edit'])) {
        $id = $_POST['id'] ?? 0;
        $name = $_POST['name'] ?? '';
        $slug = slugify($name);
    ...
    // 5. Delete Product (replacing products_delete.php)
    ...
    // 6. Upload Photo (replacing products_photo.php)
